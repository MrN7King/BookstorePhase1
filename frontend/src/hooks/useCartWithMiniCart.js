// frontend/src/hooks/useCartWithMiniCart.js
import axios from 'axios';
import { useCallback, useContext, useEffect, useState } from 'react';

// You'll need to import the MiniCart context
...
// Enhanced hook with mini cart integration
export default function useCartWithMiniCart() {
    const [cart, setCart] = useState([]);
    const [user, setUser] = useState(null);
    const [loading, setLoading] = useState(true);
    
    // Uncomment this when you add the MiniCartProvider
    ...
    // Enhanced add/update item with mini cart integration
    const addOrUpdateItem = useCallback(async (product, quantity = 1) => {
        ...
        const wasInCart = cart.find(i => i.id === productId);
        ...
        if (!user) {
            ...
            try {
                ...
                return; // Don't show mini cart on error
            }
        }

        // Show mini cart with recently added item (only for new items, not quantity updates)
        if (!wasInCart) {
            // Uncomment when you add the MiniCartProvider
    ...
    }, [user, cart]); // Added cart to dependencies for wasInCart check
