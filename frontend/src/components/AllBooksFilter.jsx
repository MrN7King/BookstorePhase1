...
// --- Pass the `genres` prop from the parent component ---
export const AllBooksFilter = ({ onClose, onApplyFilters, initialFilters = {}, genres = [], showCategories = true }) => {
    const [searchQuery, setSearchQuery] = useState(initialFilters.search || '');
    const [priceRange, setPriceRange] = useState(initialFilters.priceRange || [MIN_BOOK_PRICE, MAX_BOOK_PRICE]);
    const [selectedLanguages, setSelectedLanguages] = useState(initialFilters.language?.split(',') || []);
    const [selectedFormats, setSelectedFormats] = useState(initialFilters.format?.split(',') || []);
    const [selectedCategories, setSelectedCategories] = useState(initialFilters.categories?.split(',') || []);
    ...
    // Sync component state with initialFilters prop
    useEffect(() => {
        setSearchQuery(initialFilters.search || '');
        setPriceRange(initialFilters.priceRange || [MIN_BOOK_PRICE, MAX_BOOK_PRICE]);
        setSelectedLanguages(initialFilters.language?.split(',') || []);
        setSelectedFormats(initialFilters.format?.split(',') || []);
        setSelectedCategories(initialFilters.categories?.split(',') || []);
    }, [initialFilters]);
    ...
    const handleCheckboxChange = useCallback((item, setter) => {
        setter((prevSelected) =>
                        ...
                        {LANGUAGES.map((language) => (
                            <label key={language} className="flex items-center cursor-pointer">
                                <input
                                    ...
                                    onChange={() => handleCheckboxChange(language, setSelectedLanguages)}
                                />
                        ...
                        {BOOK_FORMATS.map((format) => (
                            <label key={format} className="flex items-center cursor-pointer">
                                <input
                                    ...
                                    onChange={() => handleCheckboxChange(format, setSelectedFormats)}
                                />
                ...
                <div className="mb-0">
                    ...
                    {isCategoriesOpen && (
                        <div id="categories-section-books" className="mt-4 space-y-3">
                            {/* The change is here: mapping over the `genres` prop */}
                            {genres.map((genre) => (
                                <label key={genre._id} className="flex items-center cursor-pointer">
                                    <input
                                        type="checkbox"
                                        className="form-checkbox h-5 w-5 text-blue-600 rounded-md focus:ring-blue-500"
                                        checked={selectedCategories.includes(genre.name)}
                                        onChange={() => handleCheckboxChange(genre.name, setSelectedCategories)}
                                    />
